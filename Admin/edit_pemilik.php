...
<body>
    ...
    include "../config.php";
    $query = mysqli_query($connection, "SELECT * FROM pemilik_mobil WHERE no_ktp_pemilik='$no_ktp_pemilik'");
            ...
            <div style="border-color: #F9E16F" class="card">
                    <div style="background-color:#fcf8e3; border-color:#F9E16F" class="panel-heading panel-custom "><h3 style="color:#CBA906" class="text-center">EDIT</h3></div>
                        <div style="border-color: #F9E16F" class="panel-body panel-custom">
                            <form method="POST" action="update_pemilik.php" class="needs-validation" novalidate>
                                <div class="form-group">
                                    <label for="nama">Nama</label>
                                    <input type="text" name="nama_pemilik" class="form-control form-control-sm" id="nama_pemilik" placeholder="" autofocus="on" value="<?php echo $row['nama_pemilik'];?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="Email">Email</label>
                                    <input type="text" name="email_pemilik" class="form-control form-control-sm" id="email_pemilik" placeholder="" value="<?php echo $row['email_pemilik'];?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="no_ktp">No. KTP</label>
                                    <input type="text" name="no_ktp_pemilik" class="form-control form-control-sm" id="no_ktp_pemilik" placeholder="" value="<?php echo $row['no_ktp_pemilik'];?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="no_telp">No. Telepon</label>
                                    <input type="text" name="no_telp_pemilik" class="form-control form-control-sm" id="no_telp_pemilik" placeholder="" value="<?php echo $row['no_telp_pemilik'];?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="alamat">Alamat</label>
                                    <input type="text" name="alamat_pemilik" class="form-control form-control-sm" id="alamat_pemilik" placeholder="" value="<?php echo $row['alamat_pemilik'];?>" required>
                                </div>
                                <form>
                                <button type="submit" style="font-weight: 500" class="btn btn-warning btn-block">Simpan</button>
                                <a href="tambah_pemilik.php" style="font-weight: 450" class="btn btn-dark btn-block">Batal</a>
                            </form>
                        </div>
                </div>
            </div>
	                    ...
	                    <?php if ($query = $connection->query("SELECT * FROM pemilik_mobil order by id_pemilik")): ?>
                            ...
                            <th scope="row"><?=$no++?></th>
	                            ...
	                            <td class="hidden-print">
	                                <div class="btn-group">
	                                    <a href="edit_admin.php?no_ktp_pemilik=<?php echo $row ['no_ktp_pemilik'];?>" style="font-weight: 500" class="btn btn-warning btn-sm">Edit</a>
	                                    <a href="hapus_admin.php?no_ktp_pemilik=<?php echo $row ['no_ktp_pemilik'];?>" style="font-weight: 500" class="btn btn-danger btn-sm">Hapus</a>
	                                </div>
	                            </td>
	                        </tr>
	                        <?php endwhile ?>
