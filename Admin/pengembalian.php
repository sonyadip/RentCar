...
<body>
    ...
    <div class="row">
        ...
        <h3 class="text-center">Data Pengembalian</h3></div>
                    ...
                    include "../config.php";
                        ...
                        // Langkah 1. Tentukan batas, cek halaman & posisi data
                            ...
                            if(isset($_POST["cari"])){
                                ...
                                $query = "SELECT pelanggan.nama, mobil.no_mobil, mobil.status, transaksi.tgl_sewa, detail_transaksi.id_transaksi, transaksi.id_transaksi, transaksi.lama, (harga*lama) as total_harga 
                                        FROM pelanggan
                                ...
                                $query = "SELECT pelanggan.nama, pelanggan.no_ktp, pelanggan.no_telp, pelanggan.email, mobil.harga, mobil.no_mobil, mobil.nama_mobil, mobil.status, mobil.merk, transaksi.tgl_sewa, detail_transaksi.id_transaksi, transaksi.id_transaksi, transaksi.lama, (harga*lama) as total_harga 
                                        FROM pelanggan
                                        INNER JOIN transaksi ON transaksi.no_ktp = pelanggan.no_ktp
                                        INNER JOIN mobil ON transaksi.no_mobil = mobil.no_mobil
                        ...
                        while($row = mysqli_fetch_array($data)){
                            ...
                            <th scope="row"><?=$no++?></th>
                                ...
                                <td><?=$row['lama']?> Hari</td>
                                <td><?=$ubahtgl2?></td>
                                <td><a class="badge badge-<?=($row['status']) ? "success" : "danger" ?>"><?=($row['status']) ? "Sudah Konfirmasi" : "Belum Konfirmasi" ?></a></td>
	                            <td class="hidden-print">
	                                <div class="btn-group">
                                    <?php 
                                        if($status == '0') { ?>
	                                    <a onclick="return confirm('Apakah anda yakin ingin konfirmasi?')" href="pengembalian_aksi.php?id_transaksi=<?php echo $row ['no_mobil'];?>" style="font-weight: 500" class="btn btn-danger btn-sm">Konfirmasi</a>
                                        <?php }
                                        else {?>
                                            <b><a>Berhasil</a></b>
                                        <?php } ?>
